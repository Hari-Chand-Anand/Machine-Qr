  ...
  return (
    <main className="min-h-dvh p-4 md:p-8">
      <div className="mx-auto max-w-lg space-y-4">
        <header className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-5">
          <div className="flex items-center gap-3">
            {envPublic.logoUrl ? (
              ...
              <div className="h-10 w-10 rounded-xl bg-white text-zinc-950 grid place-items-center font-bold">
                {envPublic.companyName.slice(0, 1).toUpperCase()}
              </div>
            )}

            <div>
              <div className="text-sm text-zinc-300">{envPublic.companyName}</div>
              <h1 className="text-xl font-semibold leading-tight">{machine.name}</h1>
              <div className="text-sm text-zinc-300">
                Machine ID:{" "}
                <span className="text-white font-medium">{machine.machineId}</span>
              </div>
            </div>
          </div>
        </header>
        ...
        <div className="grid gap-3">
          {driveLink ? (
            <a href={driveLink} target="_blank" rel="noreferrer" className="block">
              <Card className="p-5 hover:bg-zinc-900/60 transition">
            ...
            <Card className="p-5 border border-zinc-800 bg-zinc-900/40">
              <div className="text-lg font-semibold">Machine Specs & Catalog</div>
              <div className="text-sm text-zinc-300">No drive link added yet.</div>
            </Card>
          )}

          {sheetsLink ? (
            <a href={sheetsLink} target="_blank" rel="noreferrer" className="block">
              <Card className="p-5 hover:bg-zinc-900/60 transition">
                <div className="flex items-center gap-4">
                  <div className="h-12 w-12 rounded-2xl bg-white/10 grid place-items-center">
            ...
            <Card className="p-5 border border-zinc-800 bg-zinc-900/40">
              ...
              <div className="text-sm text-zinc-300">No sheets link added yet.</div>
            </Card>
          )}
